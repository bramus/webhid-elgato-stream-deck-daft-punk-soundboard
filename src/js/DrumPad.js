class DrumPad {
	...
	bindEvents = () => {
		document.addEventListener('keydown', (e) => {
			const $button = document.querySelector(`button[data-page-index="${this.activePageIndex}"][data-keycode="${e.code}"]`);
			if ($button) {
				$button.focus();
				$button.click();
			}
	...
	render = () => {
		this.targetNode.innerHTML = `
			<div class="streamdeck">
				${this.buttonConfig.map((pageButtons, pageIndex) => {
					return [...Array(15)].map((_, index) => {
						const buttonConfig = pageButtons[index] ?? null;
						
						if (buttonConfig) {
							return `
								<button class="button--label" data-page-index="${pageIndex}" data-keycode="${buttonConfig.keyCode ?? ''}" data-button-index="${index}" style="--text-color:${buttonConfig.textColor ?? ''};--bg-color:${buttonConfig.bgColor ?? ''};">
									${(buttonConfig.label ?? false) ? `<span>${buttonConfig.label}</span>` : ''}
									${(buttonConfig.audio ?? false) ? `<audio id="${buttonConfig.keyCode}" src=${buttonConfig.audio} preload="auto"></audio>` : ''}
									${(buttonConfig.image ?? false) ? `<img src="${buttonConfig.image}" alt="" title="" width="${buttonConfig.width}"  height="${buttonConfig.height}" />` : ''}
								</button>
							`;
						} else {
